...
version: 1.2.0+alrcatraz.1.0.0.angelia.0.0.0
author: alrcatraz
platforms: [linux]
...
```
GitHub (public)
  └── generic VCS skills (platform ops, routing, public references)
        ↓ + personal configuration
Gitea (private, shared across machines)
  ├── everything above
  ├── personal workflow preferences (default branch strategy, commit templates)
  ├── personal project references (private repo paths, aliases)
  ├── custom CI/CD configuration (private runner addresses, LAN mirrors)
  └── author-specific defaults
        ↓ + machine/agent information
Private copy (one per agent machine, never shared)
  ├── everything above
  ├── local project paths (actual locations under `~/Projects/`)
  ├── notes on this machine's Hermes profile configuration
  ├── local credential-store paths (KeePassXC database location, GPG key refs)
  └── machine-specific troubleshooting history

The development copy owns maintenance and pushing:
  ~/Projects/astra/astra-vcs-assist/
  ├── git push gitea main        →  Gitea (full tower, incl. personal config)
  └── git push origin up/public:main  →  GitHub (sanitised projection only)

A private copy pulls from Gitea, then layers machine-local information:
  ~/.astra/repos/astra-vcs-assist/
  ├── git pull gitea main        ← latest skills + personal configuration
  └── extra local-only notes (never pushed to any remote)
```

**Workflow:**

1. **Development copy** does daily work → `git push gitea main` (personal config included) → Gitea
2. **Development copy** publishes the public view → derive `up/public` from the full branch (strip `gitea/` and personal config), then `git push origin up/public:main` → GitHub
3. **Private copies** sync → `git pull gitea main` → skills + personal configuration from Gitea
4. **Private copies** layer machine-specific information (paths, profile, credential locations) → local only, never pushed
5. A generally useful improvement discovered on a private copy → route it through the development copy, or push directly back to Gitea

> **Gitea as backup:** shared skills + personal configuration are fully backed up on Gitea. Rebuilding an agent machine is `git clone` + recreate symlinks; machine-specific information is NOT on Gitea — restore it from local archives.
